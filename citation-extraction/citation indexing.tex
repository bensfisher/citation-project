...
\begin{enumerate}
\item Get data on journals from JCR. For each journal, try to download number of articles published and impact factor data for each year going back as far as you can.
\item Experiment with title searches on WoS
\item Follow up with Kyle about access to WoS
...
\item What is the rate of article recovery if we use WoS matched records rather than ParScit matched records (i.e., re-do Figure 4 throwing out parscit-discovered records that cannot be found on WoS)? (BF) 
...
To better understand how the ParsCit sample compares to the hand-coded sample, we generate 5 year impact factors for journals using citation counts from the two samples. There may be bias in ParsCit's indexing process if citations from some journals are overselected in comparison to the hand-coded data. The impact factors are calculated by dividing the total number of articles from a journal cited by RIAs over a five year period (2008-2012 in this case) by the total number of articles published by a journal in the same five year period. Figure~\ref{fig:impact_scatter2} plots the hand-coded impact factors against the ParsCit impact factors, while Figure~\ref{fig:rankings_scatter} plots the corresponding journal rankings based on the two measures. We also calculate Kendall's rank coefficient to measure the correlation between the two rankings. There is a correlation of approximately $.52$. \\ 
...
\begin{figure}
\begin{center}
\includegraphics[height=8cm]{rankings_scatter.png}
\caption{Hand-coded vs. ParsCit Journal Rankings}
\label{fig:rankings_scatter}
\end{center}
\end{figure}

\begin{figure}
\begin{center}
\includegraphics[height=8cm]{impact_factor_scatter2.png}
\caption{Hand-coded vs. ParsCit Impact Factors}
\label{fig:impact_scatter2}
\end{center}
\end{figure}
